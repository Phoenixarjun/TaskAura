...
app.use(
  cors({
    origin: [
      ...
      "https://taskaura-epbivpexy-naresh-b-as-projects.vercel.app",
    ...
    const server = app
      .listen(PORT, () => {
        ...
        if (dbConnected) {
          ...
          console.log(
            "📝 Create .env file with MONGODB_URI to enable full features"
          );
      ...
      .on("error", (error) => {
        if (error.code === "EADDRINUSE") {
          ...
          const alternativeServer = app
            .listen(alternativePort, () => {
              console.log(
                `🚀 TaskAura API server running on port ${alternativePort}`
              );
              console.log(
                `📊 Environment: ${process.env.NODE_ENV || "development"}`
              );
              console.log(
                `🔗 Health check: http://localhost:${alternativePort}/health`
              );
              console.log(
                `🔗 Test endpoint: http://localhost:${alternativePort}/test`
              );
            ...
            .on("error", (altError) => {
              console.error(
                `❌ Alternative port ${alternativePort} also in use`
              );
              console.log(
                "🔧 Please stop existing processes or change PORT in .env file"
              );
              process.exit(1);
            });
        } else {
